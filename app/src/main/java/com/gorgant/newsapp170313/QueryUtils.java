...
public class QueryUtils {
    ...
    public static List<NewsArticle> fetchArticleData(String requestUrl) {
        ...
        //An artificial delay to ensure loading animation is displayed
        try {
            Thread.sleep(2000);
    ...
    private static List<NewsArticle> extractFeatureFromJson(String newsArticleJSON) {
        ...
        try {
            ...
            JSONObject preBaseJsonResponse = new JSONObject(newsArticleJSON);

            // Pull the results from the prebaseresponse
            JSONObject baseJsonResponse = preBaseJsonResponse.getJSONObject("response");
            ...
            for (int i = 0; i < newsArticleArray.length(); i++) {
                ...
                NewsArticle newsArticle = new NewsArticle(title,section,date,url);
